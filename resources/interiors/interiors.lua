...
function takeProductos(interiorID, cantidad)
	if isTienda(interiorID) == true then
		local interior = interiors[ interiorID ]
		if interior then
			if exports.sql:query_free("UPDATE interiors SET productos = "..(cantidad-interior.productos).." WHERE interiorID = "..interiorID) then
				interior.productos = interior.productos-cantidad
				return true
			else
				return false
			end
...
-- Ojo con habilitar esto, porque un trabajador de la empresa de repartos podría comprarse a sí mismo infinitamente, y se sacaría un dineral.
