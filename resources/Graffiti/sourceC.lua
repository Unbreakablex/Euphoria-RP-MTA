...
local panelMenu = {
	{"Nuevo", tocolor(217, 83, 79), "new"},
	{"Atras", tocolor(240, 173, 78, 230), "undo"},
	{"Adelante", tocolor(240, 173, 78, 230), "redo"},
	{"Guardar", tocolor(21, 219, 114), "save"},
	{"Colorselector", tocolor(50, 179, 239, 230), "palette", false,
		activeWhen = function()
	...
	{"Gruesor del cepillo", tocolor(50, 179, 239, 230), "brushsize", false,
		activeWhen = function()
	...
	{"Tema (claro/oscuro)", tocolor(50, 179, 239, 230), "theme"},
	{"Guardar graffitis", tocolor(21, 219, 114), "savedgraffitis", 10,
		activeWhen = function()
	...
	{"Cargar imagen desde URL", tocolor(244, 184, 235), "url_image", 11,
		activeWhen = function()
...
addCommandHandler("graffitisize",
	function (command, size)
		if not tonumber(size) or tonumber(size) <= 0 or tonumber(size) > 3 then
			outputChatBox("Sintaxis /" .. command .. " [Size (1 - 3)]", 255, 255, 255, true)
		else
			graffitiSize = tonumber(size)
			outputChatBox("El tamaño del graffiti cambié con éxito a #e56b6b" .. graffitiSize .. ".", 255, 255, 255, true)
		end
	end
)

addCommandHandler("graffitiadmin",
	function ()
		adminMode = not adminMode
	end
)

addCommandHandler("glimpiar",
	function ()
		graffitiCleanMode = not graffitiCleanMode
		
		if graffitiCleanMode then
			outputChatBox("El modo de limpieza de graffiti ahora está habilitado.", 255, 255, 255, true)
		else
			outputChatBox("El modo de limpieza de graffiti ahora está desactivado.", 255, 255, 255, true)
		end
	end
)
				...
				if deltaTime >= 3000 then
					if utf8.len(path) > 0 then
						...
						if fileExtension and fileExtension == ".png" or fileExtension == ".jpg" or fileExtension == ".jpeg" or fileExtension == ".bmp" then
							...
							outputChatBox("Empezaste a descargar la imagen seleccionada. !Espere el resultado!", 255, 255, 255, true)
						else
							outputChatBox("La extensión de ruta seleccionada no es válida!", 255, 255, 255, true)
						end
					else
						outputChatBox("Primero agregue la URL de la imagen!", 255, 255, 255, true)
					end
				else
					outputChatBox("Please wait #e56b6b" .. 3 - math.floor(deltaTime / 1000) .. " second#ffffff to the next download.", 255, 255, 255, true)
				end
			else
				outputChatBox("Ya hay una descarga en curso!", 255, 255, 255, true)
			end
		end
	end
)
...
addEventHandler("onClientReceiveDownloadedImage", getRootElement(),
	function (imagePixels, errno)
		if errno == 0 then
			...
			outputChatBox("La imagen seleccionada se guardó correctamente.", 255, 255, 255, true)
			outputChatBox("Puede llegar a la imagen en el panel 'Graffitis guardados'.", 255, 255, 255, true)
		else
			outputChatBox("La imagen seleccionada no está disponible!", 255, 255, 255, true)
		end
	...
	function (fromClient, pixels, data)
		...
		if graffitiFile then
			...
			if fromClient == localPlayer then
				...
				outputChatBox("El graffiti se completó con éxito.", 255, 255, 255, true)
			end
		end
	end
)
	...
	function (weapon, _, _, hitX, hitY, hitZ)
		...
		if lastDrawedPixels and not startedGraffiti.isActive and not startedGraffiti.actionDisabled and #drawingSteps >= 2 and getTickCount() >= nextSprayingAfterTick then
			if getTickCount() >= tryToSprayGraffitiTick then
				...
				outputChatBox("Empezaste a pintar un nuevo graffiti!", 255, 255, 255, true)
			end
				...
				if button == "left" and state == "up" then
					if string.find(selected3DButton, "trash|") then
						...
						if serverGraffitis[fileName].isProtected then
							outputChatBox("No puedes eliminar este graffiti porque está protegido.", 255, 255, 255, true)
							return
						end
					...
					elseif string.find(selected3DButton, "clean|") then
						...
						if not getPedOccupiedVehicle(localPlayer) then
							if not isTimer(cleaningGraffitiTimer) then
								if serverGraffitis[fileName].isProtected then
									outputChatBox("No puedes limpiar este graffiti porque está protegido.", 255, 255, 255, true)
									return
								end
								...
								outputChatBox("Ya tienes un proceso de limpieza de grafitis activo.", 255, 255, 255, true)
							end
						else
							outputChatBox("No puede usar esta función mientras estás sentado en un vehículo.", 255, 255, 255, true)
						end
					end
				end
			end
...
function cleanGraffiti(fileName)
	...
	outputChatBox("Finalizó con éxito la limpieza de grafitis.", 255, 255, 255, true)
end
...
function renderTheEditor()
	...
	if promptDialog.isActive then
		...
		dxDrawText(promptDialog.text .. "\n\n\nApliar #32b3ef[ENTER]#ffffff - Cancelar #32b3ef[BACKSPACE]", panelPosX, panelPosY, panelPosX + panelWidth, panelPosY + panelHeight, tocolor(255, 255, 255), 1.0, mainFont, "center", "center", false, false, false, true)
	end
end
