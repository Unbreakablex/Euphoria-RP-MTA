...
function showPokerDisplay ()
	...
	if gWindow["pokerCommunity"] then
		...
		gWindow["pokerCommunity"] = guiCreateWindow(screenwidth/2-450/2,screenheight/2-256/2,450,259,"Mesa de Poker",false)
		...
		gButton["pokerCheck"] = guiCreateButton(0.2443+0.17*-1+0.0251*-1,0.695,0.17,0.1351,"Pasar",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerCheck"],0)
			...
			function ()
				if guiGetAlpha ( source ) == 1 and source == gButton["pokerCheck"] and block then
					if pokerCurBets == 0 or pokerCurBets == curOwnBet then
						...
						outputChatBox ( "¡No puedes pasar ahora! Iguala o sube la apuesta.", 125, 0, 0 )
					end
				end
			end
		)
		...
		gButton["pokerFold"] = guiCreateButton(0.2443+0.17+0.0251,0.695,0.17,0.1351,"No ir",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerFold"],0)
		...
		gButton["pokerRaise"] = guiCreateButton(0.2443+0.17*2+0.0251*2,0.695,0.1005,0.139,"Subir/Igualar",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerRaise"],0)
				...
				if guiGetAlpha ( source ) == 1 and source == gButton["pokerRaise"] and block then
					...
					if raiseAmount >= 10 then
						...
						if pokerCurOwnChips >= raiseAmount then
							...
							outputChatBox ( "¡No tienes suficientes fichas!", 125, 0, 0 )
						end
					else
						outputChatBox ( "¡Necesitas 10 fichas al menos!", 125, 0, 0 )
					end
				end
			end
		)
		
		gButton["pokerLeave"] = guiCreateButton(0.7717,0.8417,0.18491,0.1236,"Salir",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerLeave"],1)
		...
		gLabel["pokerInfo1"] = guiCreateLabel(0.0205,0.8533,0.0571,0.0579,"Bote:",true,gWindow["pokerCommunity"])
		...
		gLabel["pokerInfo2"] = guiCreateLabel(0.1702,0.8456,0.121,0.0656,"Jugadores:",true,gWindow["pokerCommunity"])
