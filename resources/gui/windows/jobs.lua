...
windows.jobs_trailero = {
	{
		type = "label",
		text = "SA Trucking Association",
		font = "bankgothic",
		alignX = "center",
	},
	{
		...
		panes = 
		{
			{	
				image = "images/okay.png",
				title = "Conseguir empleo",
				text = "¿Necesitas dinero? Te damos la oportunidad de \nconseguirlo conduciendo camiones.",
				onHover = function( cursor, pos )
					dxDrawRectangle( pos[1], pos[2], pos[3] - pos[1], pos[4] - pos[2], tocolor( unpack( { 75, 255, 75, 31 } ) ) )
				end,
				onClick = function (key)
					if key == 1 then
						triggerServerEvent("onConseguirEmpleo", getLocalPlayer(), "trailero")
					end
				end,
			},
			{	
				...
				onClick = function (key)
					triggerServerEvent("onDejarEmpleo", getLocalPlayer(), "trailero")
				end,
				onCreate = function ()
				table.insert( windows.jobs_trailero, closeButton )
				end,
			}
		}
	},
