...
function abrirVentanaAsistenciaV2()
	if ventanaAsis == false then
		...
		ventanaAsis = true
		vAsistenciaGeneral = guiCreateWindow(527/1920*x, 291/1080*y, 867/1920*x, 530/1080*y, "Panel de asistencia general - Exotic - Juego de Rol", false)
		...
		lInicioPanel = guiCreateLabel(31/1920*x, 25/1080*y, 760/1920*x, 105/1080*y, "Bienvenid@ al panel de asistencia general de Exotic - Juego de Rol. Desde aquí podrás ver los conceptos de rol, los comandos del servidor, reportar a jugadores, ¡y muchos trámites más!\n\nSelecciona una categoría (ej. Comandos) para obtener más información. En caso de cualquier problema, siempre puedes dirigirte a un staff vía IG mediante /staff o vía IG y web a través del CAU.", false, tabInicio)
		...
		labelCAU = guiCreateLabel(18/1920*x, 13/1080*y, 774/1920*x, 225/1080*y, "Desde aquí podrás acceder al CAU, Centro de Atención al Usuario. La mayoría de trámites se realizarán en este CAU, siendo un medio directo, seguro y privado de comunicación entre el usuario y el staff.\n\nUna vez vincules tu cuenta IG con tu cuenta del foro, tendrás acceso al CAU y desde él podras abrir casos. Un caso hace referencia a una queja, una petición algo peculiar, no estar de acuerdo con una sanción recibida, y diversos trámites puntuales que serán anunciados vía foro.\n\nCon el fin de garantizar el correcto funcionamiento de este servicio, al pulsar en el botón 'Acceder al CAU' aparecerá una ventana pidiendo permisos para poder acceder a *.exotic-rp.foroactivo.com. Es necesario que marques la casilla de Remeber decision y des en Allow para permitirlo. En caso contrario, no podrás acceder al CAU vía IG.", false, tabCAU)
        guiLabelSetHorizontalAlign(labelCAU, "left", true)
        bAccesoCAU = guiCreateButton(250/1920*x, 255/1080*y, 310/1920*x, 60/1080*y, "Acceder al CAU", false, tabCAU)
		...
		lGestiones = guiCreateLabel(18/1920*x, 16/1080*y, 776/1920*x, 40/1080*y, "Desde aquí obtendrás información sobre cómo realizar ciertas gestiones administrativas: eliminar personajes, cambiar contraseñas, entre otros. \nSimplemente selecciona un trámite para obtener información.", false, tabOGestiones)
		guiLabelSetHorizontalAlign(lGestiones, "left", true)
		gridGestiones = guiCreateGridList(22/1920*x, 60/1080*y, 188/1920*x, 254/1080*y, false, tabOGestiones)
		...
		guiGridListSetItemText(gridGestiones, 10, 1, "Ingresar a Discord", false, false)
...
function regularBotonesAsistencia()         
	...
	elseif source == gridGestiones then
		...
		if gestionSel == "Cambiar mi clave" then
			lRespuestaOGestiones = guiCreateLabel(221/1920*x, 64/1080*y, 573/1920*x, 348/1080*y, "Para cambiar tu clave, simplemente utiliza el comando /cambiarclave [nueva clave de tu cuenta]\n\nTen en cuenta que por seguridad sólo se puede cambiar la clave de tu cuenta si estás desde el PC que se creó.\n\nTambién recordarte que el cambio de clave cambiará tu clave IG, pero no la clave de tu cuenta en foro. Si quieres cambiar tu clave de foro, deberás de hacerlo desde el foro.", false, tabOGestiones)
			guiLabelSetHorizontalAlign(lRespuestaOGestiones, "left", true)
		elseif gestionSel == "Anular un préstamo" then
			lRespuestaOGestiones = guiCreateLabel(221/1920*x, 64/1080*y, 573/1920*x, 348/1080*y, "Para anular tu préstamo, debes de seleccionar primero el tipo de préstamo que es (vehículo o interior) y hacer clic en continuar.\n\nTen en cuenta que la anulación de un préstamo conllleva la pérdida del objeto relacionado con él (vehículo o interior), y que el dinero que ya has pagado NO se devolverá.\n\nPor último, recordarte que la anulación es instantánea, y no hay vuelta atrás.", false, tabOGestiones)
			guiLabelSetHorizontalAlign(lRespuestaOGestiones, "left", true)

			cAceptoCondAnulPrest = guiCreateCheckBox(90/1920*x, 212/1080*y, 15/1920*x, 15/1080*y, "", false, false, lRespuestaOGestiones)
			lAceptoCondicionesPrest = guiCreateLabel(115/1920*x, 212/1080*y, 330/1920*x, 18/1080*y, "He leído y acepto las condiciones de anulación de préstamo.", false, lRespuestaOGestiones)

			bAnularPrestamoVeh = guiCreateButton(272/1920*x, 318/1080*y, 167/1920*x, 57/1080*y, "Anular préstamo de mi vehículo", false, tabOGestiones)
			bAnularPrestamoInterior = guiCreateButton(579/1920*x, 318/1080*y, 167/1920*x, 57/1080*y, "Anular préstamo de mi interior", false, tabOGestiones)
		...
		elseif gestionSel == "Eliminar mi cuenta" then
			...
			guiLabelSetHorizontalAlign(lRespuestaOGestiones, "left", true)
		elseif gestionSel == "Ingresar a Discord" then
			lRespuestaOGestiones = guiCreateLabel(221/1920*x, 64/1080*y, 573/1920*x, 248/1080*y, "A la hora de entrar al servidor se recomienda ingresar al Discord al instante. Al entrar procura tener el mismo nombre que tu cuenta.", false, tabOGestiones)
			guiLabelSetHorizontalAlign(lRespuestaOGestiones, "left", true)
	...
	elseif source == bAccesoCAU then
		...
		--else
			requestBrowserDomains({ "Foro", "exotic-rp.foroactivo.com" })
...
• Si te hacen un CK, como se dijo anteriormente mataron a tu personaje, por lo cual por razones logicas tu personaje muerto no puede saber quien fue, ya que está muerto.]])
		elseif conceptoSel == "Troll" then
			guiSetText(memoConceptosRol, [[En Exotic - Juego de Rol nos queremos enfocar en que los usuarios se diviertan y esto es dándoles un poco de libertad. ¿A que nos referimos con libertad?

Si estas con tus amigos en un lugar donde no molesten a nadie pueden hacer lo que quieran siempre que no involucre a otros usuarios en su rol poco rolero, en el momento que otro usuario es involucrado y tanto tu como tu grupo siguen con la actitud “Troll” el equipo administrativo se verá en la obligación de repartir sanciones a quienes se la ganen.

También hay que recalcar que no se permite esta actitud en lugares de rol público, si están en un interior o en una zona lejana ya no nos molesta.

Troll: Es la actitud que pueden tomar ciertos usuarios con el fin de arruinar la experiencia de Rol de otros.]])
		elseif conceptoSel == "AA" then
			guiSetText(memoConceptosRol, [[AA:

Admin Abuser es el término utilizado para las faltas administrativas cometidas por un staff ante un usuario, con el único fin de sacarle provecho o ventaja.

En Exotic – Juego de Rol no se permite bajo ninguna circunstancia que un Staff haga uso de sus poderes para perjudicar a un usuario, como también para darse beneficios tanto a el como a su grupo de amigos. El staff que sea encontrado haciendo AA será retirado de su cargo en el instante.
]])
		elseif conceptoSel == "CJ" then
			guiSetText(memoConceptosRol, [[CJ:
				
				Es la acción de sacar a un usuario de su vehículo sin rol alguno aprovechando el sistema que trae por defecto el juego, el hacer este acto es considerado como Troll y a su vez es una falta de Rol que conlleva sanciones.]])
		end
	end
end