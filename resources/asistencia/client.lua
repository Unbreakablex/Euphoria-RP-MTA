...
function finalizarAsistencia()
	...
	outputChatBox("¡Gracias por haber usado la Asistencia al Usuario de Exotic - Juego de Rol!", 0, 255, 0)
end

function regeneraVentana()
	destroyElement(vAsistencia)
	vAsistencia = guiCreateWindow(428*x/1366, 188*y/768, 583*x/1366, 456*y/768, "Asistencia al Usuario - Exotic - Juego de Rol.", false)
...
function mostrarGUIAsistencia()
	if exports.players:isLoggedIn() then
		if ventanaAsis == false then
			...
			vAsistencia = guiCreateWindow(428*x/1366, 188*y/768, 583*x/1366, 456*y/768, "Asistencia al Usuario - Exotic - Juego de Rol", false)
			guiWindowSetSizable(vAsistencia, false)
			lInfo = guiCreateLabel(117*x/1366, 41*y/768, 346*x/1366, 38*y/768, "Bienvenido al sistema de Asistencia al Usuario de Exotic - Juego de Rol. \nPor favor, selecciona en la lista el problema que tienes.", false, vAsistencia)
			rSoyNuevo = guiCreateRadioButton(44*x/1366, 102*y/768, 463*x/1366, 18*y/768, "Soy nuevo en el servidor, ¿cómo se juega?", false, vAsistencia)
...
function regularBoton()
	...
	elseif source == bContinuar then
		...
		elseif s_rNecesitoStaff == true then
			lInfo = guiCreateLabel(28*x/1366, 45*y/768, 527*x/1366, 129*y/768, "Te informamos de que acabamos de lanzar una alerta general a todos los staffs de servicio. Esperamos que le hayas llamado con razón, porque en caso contrario podrás ser sancionado.\n\nPor otro lado, ten en cuenta que un staff no podrá actuar ante un rol en la mayoría de los casos. Se deberá de ir al discord (https://discord.gg/dm74JqHP8k) e interponer el reporte correspondiente.\n\nGracias por confiar en Exotic - Juego de Rol. Por favor, espera a que acuda el staff.", false, vAsistencia)
			guiLabelSetHorizontalAlign(lInfo, "center", true)
