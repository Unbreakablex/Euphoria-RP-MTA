		...
		if c then
			if p[ player ].route[ p[ player ].checkpoint ] then
				...
				if p[ player ].route[ p[ player ].checkpoint ].hint then
					triggerClientEvent( player, "gui:hint", player, "Tu trabajo: Repartidor", p[ player ].route[ p[ player ].checkpoint ].hint )
				end
			end
			...
			if not n then
				...
				exports.objetivos:addObjetivo(4, exports.players:getCharacterID(player), player)
			else
...
local function createOurPedRepartidor( )
	if pedR then
		destroyElement( pedR )
	end
...
addEventHandler( "onPedWasted", resourceRoot, createOurPedRepartidor )
addEventHandler( "onResourceStart", resourceRoot, createOurPedRepartidor )
...
addEventHandler( "onElementClicked", resourceRoot,
	function( button, state, player )
		if button == "left" and state == "up" then
			...
			if getDistanceBetweenPoints3D( x, y, z, getElementPosition( source ) ) < 5 and getElementDimension( player ) == getElementDimension( source ) and source == pedR then
				triggerClientEvent("onAbrirJob", player, "repartidor")
			end
		end
	end
)