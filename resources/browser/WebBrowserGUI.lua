...
function WebBrowserGUI:constructor()
	...
	guiSetAlpha(self.m_EditAddress, 0)
	guiSetAlpha(self.m_LoadButton, 0)
	guiEditSetMaxLength(self.m_EditAddress, 250)
...
function abrirPortalCAU(uID, token1, token2)
	WebBrowserGUI.instance = WebBrowserGUI:new()
	setTimer(function(uID, token1, token2)
	WebBrowserGUI.instance:loadURL("https://exotic-rp.foroactivo.com")
	end, 2000, 1, uID, token1, token2)
end --https://login.dt-mta.com/index.php?IDS=2&IG_uID=
addEvent("onAbrirPortalCAU", true)
addEventHandler("onAbrirPortalCAU", getRootElement(), abrirPortalCAU)