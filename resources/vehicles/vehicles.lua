	...
	function( )
		...
		if result then
			for key, data in ipairs( result ) do
				...
				setVehiclePlateText(vehicle, tostring(addZero( data.numberplate, 4 )))
				...
				exports.vehicles_auxiliar:Tunning(vehicle)
		...
		if model then
			...
			if vehicle then
				...
				if vehicleID then
					...
					setElementData( vehicle, "marchas", 0 )
					setVehiclePlateText(vehicle, numberplate)
...
function createVehicleFromConce( player, model2 )
	if isElement( player ) then
		...
		if characterID then
		    local vehicle = createVehicle(tonumber(model2), 1345.4320068359, 441.28515625, 20.718496322632, 359.8681640625, 359.99450683594, 245.52894592285)
			...
			if vehicleID then
				...
				setVehiclePlateText(newVehicle, numberplate)
...
function createVehicleFromConceaereo( player, model2 )
	if isElement( player ) then
		local characterID = exports.players:getCharacterID( player )
		if characterID then
		    local vehicle = createVehicle(tonumber(model2), 1386.4365234375, 468.0771484375, 20.149862289429, 359.8681640625, 359.99450683594, 330.55892944336)
			local model = getElementModel( vehicle )
			local x, y, z = getElementPosition( vehicle )
			local rx, ry, rz = getVehicleRotation( vehicle )
			local interior = getElementInterior( vehicle )
...
addEvent("generarVehiculoAereo", true)
addEventHandler("generarVehiculoAereo", root, createVehicleFromConceaereo)

function createVehicleFromConcebotes( player, model2 )
	if isElement( player ) then
		local characterID = exports.players:getCharacterID( player )
		if characterID then
		    local vehicle = createVehicle(tonumber(model2), 52.1708984375, 194.2626953125, 0.55000001192093, 359.8681640625, 359.99450683594, 330.55892944336)
			local model = getElementModel( vehicle )
			local x, y, z = getElementPosition( vehicle )
			local rx, ry, rz = getVehicleRotation( vehicle )
			local interior = getElementInterior( vehicle )
...
addEvent("generarVehiculoBotes", true)
addEventHandler("generarVehiculoBotes", root, createVehicleFromConcebotes)
...
function toggleFreezeStatusTrailer ( thePlayer, commandName )
	local vehicleID = vehicles[ vehicle ] and vehicles[ vehicle ].vehicleID
	local playerVehicle = vehicle.ID ( thePlayer )
	if playerVehicle then
		local playerVehicle = vehicle.ID ( thePlayer )
		local sx, sy, sz = getElementVelocity (playerVehicle)
		local velocidad = (sx^2 + sy^2 + sz^2)^(0.5) 
		if tonumber(velocidad*180) <= 2 then
			...
			elseif getVehicleType(playerVehicle) == "Plane" or getVehicleType(playerVehicle) == "Helicopter" then
				exports.chat:me( thePlayer, ( isElementFrozen( playerVehicle ) and "quita" or "pone" ) .. " el seguro anti-despegue." )
				setElementFrozen ( playerVehicle, newFreezeStatus )
				elseif getVehicleType(playerVehicle) == "Trailer" then
				exports.chat:me( thePlayer, ( isElementFrozen( playerVehicle ) and "quita" or "pone" ) .. " el seguro anti-despegue." )
				setElementFrozen ( playerVehicle, newFreezeStatus )
			else
				exports.chat:me( thePlayer, ( isElementFrozen( playerVehicle ) and "quita" or "pone" ) .. " el freno de mano." )
...
addCommandHandler ( "trailer", toggleFreezeStatusTrailer  )
	...
	if vehicle then
		...
		if getVehicleType(vehicle) == "BMX" or getVehicleType(vehicle) == "Bike" then
			...
			if sicinturon == false or sicinturon == nil then
				...
				exports.chat:me(player, "se abrocha el cinturón.")
			else
				removeElementData(player, "player.cinturon")
				exports.chat:me(player, "se quita el cinturón.")
			end
		end
	end
end
