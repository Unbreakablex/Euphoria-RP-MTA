...
function showLogin(serialRegistered)
	...
	lp.register.button = dxCreateButton((sX/2 - (400*x)/2 - 280*x) + (250*x)/2 - (120*x)/2,( sY/2 - (250*y)/2 ) + (200*y),120*x,30*y,"Registrar",tocolor(0,0,0,255))
...
---400 y 160
function drawLogin()
	dxDrawImage(sX/2 - (250*x)/2,45,275*x,100*y,"img/logo.png")
    ...
    dxDrawEmptyRectangle(sX/2 - (150*x)/2,( sY/2 - (410*y)/2 ) + (260*y),150*x,30*y,tocolor(240,240,240,255),1)
...
function drawRegister()
	...
	dxDrawText2("¿No tienes cuenta? ¡Crea una!",(sX/2 - (400*x)/2) - 280*x,0,250*x,250*y,tocolor(70,119,173,255),1.2*fontsize,"arial-bl","center","bottom")
	...
	if not lp.hasAccount then               
        ...
        dxDrawEmptyRectangle((sX/2 - (400*x)/2 - 280*x) + (250*x)/2 - (120*x)/2,( sY/2 - (250*y)/2 ) + (200*y),120*x,30*y,tocolor(240,240,240,255),1)
	else
