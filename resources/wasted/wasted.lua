...
addEventHandler("onClientMuerto", getLocalPlayer(), playerDied)
...
function getKillString(player, killer, weapon, bodypart)
    ...
    if player then
        --killText = killText .. "you"
    end --if

    if killer then
        --killText = getPlayerName(killer) .. " killed " .. killText
    else
        --killText = killText .. " committed suicide "
    end 
...
addEventHandler("onClientMuerto", getLocalPlayer(),
function()
...
function createShader()
	...
	setTimer(function() destroyElement(oldFilmShader) end, 505000, 1)
...
addEventHandler("onClientNoMuerto", getLocalPlayer(),
function ()
    setTimer(function() destroyElement(oldFilmShader) end, 100, 1)
    setGameSpeed(1)
end)
