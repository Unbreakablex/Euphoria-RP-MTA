...
function casarse ( thePlayer, commandName, otherPlayer )
		...
		if thePlayer ~= other then
			if other then
				...
				if nivel < 0 or nivel2 < 0 then outputChatBox("O tú o con quien te vas a casar no tiene el nivel 3, y no podéis casaros (/objetivos).", thePlayer, 255, 0, 0) return end
				...
				if tonumber(s.casadocon) == 0 then
					if getElementDimension(thePlayer) ~= 5 then outputChatBox("(( Tienes que estar en la iglesia ))", thePlayer, 255, 0, 0) return end
					if getElementDimension(other) ~= 5 then outputChatBox("(( Tiene que estar tu pareja en la iglesia ))", thePlayer, 255, 0, 0) return end
					if exports.players:takeMoney(thePlayer, 15000 ) then
						...
						outputChatBox("[Inglés] [PCND] Reportero dice: Tenemos noticias ciudadanos de Red County!", root, 62, 184, 255)
						outputChatBox("[Inglés] [PCND] Reportero dice: Tenemos un nuevo matrimonio! , se ha casado "..getPlayerName(thePlayer):gsub("_", " ").." con "..getPlayerName(other):gsub("_", " ").." Felicidades", root, 62, 184, 255)
...
setElementDimension(padremendez,5)
---------------------