		...
		if tipo == "player" then
			...
			if arma == 0 then
				if muerto == true then
					...
					setPedAnimation(source, "wuzi", "cs_dead_guy", -1, true, false, false)
				...
				if muerto == true then
					...
					if not staff then
						if arma == 24 and getElementData(atacante, "tazerOn") then
							outputChatBox("El jugador "..getPlayerName(atacante):gsub("_", " ").." te ha tazeado con su Taser.", source, 177, 177, 177)
							outputChatBox("Has tazeado a "..getPlayerName(source):gsub("_", " ").." con tu Taer.", atacante, 177, 177, 177)
	...
	if player and getElementData(player, "muerto") == true then
		if hayMedicos() then
			...
			if not getElementData(player, "reduccNOMD") or getElementData(player, "reduccNOMD") ~= 2 then
				...
				outputChatBox("No hay médicos disponibles. Puedes reaparecer cuando sea posible.", player, 255, 0, 0)
					...
					setTimer(
						function( source )
							if isElement( source ) and exports.players:isLoggedIn( source ) then
								...
								outputChatBox("Esperamos que hayas roleado correctamente por entorno la atención y el traslado al hospital.", source, 255, 255, 255)
								outputChatBox("Además, ten en cuenta si ha sido un PK, un CK, etcétera.", source, 255, 0, 0)
								outputChatBox("Si moriste por mano de un usuario de forma 'Poco Rolera' recuerda tomar SS y reportar vida foro.", source, 255, 0, 0)
