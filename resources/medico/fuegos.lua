...
local tablafuegos = {
        ...
        { 2332.56, 93.44, 26.33, "Cerca de Pizzeria La Esquina", "Hay un vehículo echando humo, ¡seguramente ardiendo!", "regular", 401 },
         { 1844.42, 358.72, 19.66, "Frente a la estacion de Servicios de Red County a las afueras de Palomino se encendio un auto en llamas.", "regular", 401 },
...
function iniciarfuegoaleatorioAdmin()
    ...
    local fX, fY, fZ = tablafuegos[randomfire][1],tablafuegos[randomfire][2],tablafuegos[randomfire][3]
                exports.factions:sendMessageToFaction(2, "(( Radio: PCND )) CENTRAL: Se ha recibido una llamada de emergencia un posible incendio se requieren todas las unidades.",127, 127, 255 )
				exports.factions:sendMessageToFaction(2, "(( Radio: PCND )) CENTRAL: Es de gravedad: "..tablafuegos[randomfire][5].."("..tablafuegos[randomfire][6]..").",127, 127, 255 )
                exports.factions:sendMessageToFaction(2, "(( Radio: PCND )) CENTRAL: "..tablafuegos[randomfire][4].." ir de urgencia , se han enviado las cordenadas al GPS.",127, 127, 255 )
...
function cancelarfuegoiniciado (thePlayer)
	if thePlayer and hasObjectPermissionTo(thePlayer, "command.reloadpermissions", false) then
		...
		if (isFireOn) then	
			...
			--outputChatBox ("Sistema de fuegos Exotic Juego de Rol", thePlayer,255,255,0)
