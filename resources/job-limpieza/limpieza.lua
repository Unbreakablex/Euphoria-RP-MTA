	...
	if p[ player ] and p[ player ].route and p[ player ].checkpoint then
		...
		if c then
			...
			if not n then
				...
				exports.objetivos:addObjetivo(2, exports.players:getCharacterID(player), player)
			else
...
local function createOurPedLimpieza( )
	if pedL then
		destroyElement( pedL )
	end
...
addEventHandler( "onResourceStart", resourceRoot, createOurPedLimpieza )

addEventHandler( "onElementClicked", resourceRoot,
	function( button, state, player )
		if button == "left" and state == "up" then
			local x, y, z = getElementPosition( player )
			if getDistanceBetweenPoints3D( x, y, z, getElementPosition( source ) ) < 5 and getElementDimension( player ) == getElementDimension( source ) and source == pedL then
				triggerClientEvent("onAbrirJob", player, "limpieza")
			end
		end
	end
)

function ayudaPos(player)
	if not hasObjectPermissionTo( player, "command.modchat", false ) then return end
	local x, y, z = getElementPosition (player)
	outputChatBox("{ x = "..math.round ( x, 2 )..", y = "..math.round ( y, 2 )..", z = "..math.round ( z, 2 )..", stop = true },", player, 0, 255, 0)
end
addCommandHandler("apos", ayudaPos)
