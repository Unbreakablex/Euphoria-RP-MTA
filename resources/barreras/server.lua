...
b_mecas = createObject ( 9093, 2290.064453125, -123.3828125, 27.87, 0, 0, 270 ) --1804.0999755859, -1766.8000488281, 14.300000190735
e_mecas = 0

function f_mecanicos (player)
	if exports.factions:isPlayerInFaction( player, 3 ) then
		if not isElementInRange(player, 2290.08203125, -122.5830078125, 26.453544616699, 20) then outputChatBox("(( El mando no tiene tanto alcance ))", player, 255, 0, 0) return end
		if e_mecas == 0 then
			e_mecas = 2
			exports.chat:me( player, "presiona el botón y abre el portón." )
			moveObject ( b_mecas, 1500, 2290.064453125, -123.3828125, 29.87, 0, 90, 0)
			setTimer( function () 
			e_mecas = 1 end, 2500, 1, player)
		elseif e_mecas == 1 then
			moveObject ( b_mecas, 1500, 2290.064453125, -123.3828125, 27.87, 0, -90, 0)
...
b_mecas2 = createObject ( 9093, 2290.064453125, -123.3828125, 24.33, 0, 0, 270 ) --1789.6999511719, -1766.8000488281, 14.300000190735
e_mecas2 = 0
...
function f_mecanicos2 (player)
	if exports.factions:isPlayerInFaction( player, 3 ) then
		...
		if e_mecas2 == 0 then
			e_mecas2 = 2
			exports.chat:me( player, "presiona el botón y abre el portón." )
			moveObject ( b_mecas2, 1500, 2290.064453125, -123.3828125, 19.87, 0, 90, 0)
			setTimer( function () 
			e_mecas2 = 1 end, 2500, 1, player)
		elseif e_mecas2 == 1 then
			moveObject ( b_mecas2, 1500, 2290.064453125, -123.3828125, 24.33, 0, -90, 0)
			exports.chat:me( player, "presiona el botón y cierra el portón." )
			e_mecas2 = 2
			setTimer( function () 
...
addCommandHandler( "btaller", f_mecanicos2)


-- Puertas policia
barreraPol = createObject ( 968, 2425.2, 52, 26.2, 0, 90, 0 )
markerPol = createMarker (2428.88, 52.18, 26.45, "cylinder", 7.5, 255, 0, 0, 0 )
esbarpol = 0
...
barreraGOB = createObject ( 968, 2254.4448242188, -87.883399963379, 26.15, 0, 90, 180 )
...
function toggleBarreraGOB(element)
	...
	if exports.factions:isPlayerInFaction( player, 5 ) then
		if esbargob == 0 then
			exports.chat:me( player, "presiona el botón y sube la barrera." )
			moveObject ( barreraGOB, 1200, 2254.4448242188, -87.883399963379, 26.15, 0, -90, 0)
			esbargob = 1
			setTimer( function () 
			moveObject ( barreraGOB, 1200, 2254.4448242188, -87.883399963379, 26.15, 0, 90, 0)
			end, 5000, 1, player)
...
barreraPT = createObject ( 968, 1416.1, 420, 19.6, 0, 270, 334.5 )
soportePT = createObject ( 966, 1416.1, 420, 18.8, 0, 0, 334.5 )
...
function toggleBarreraPT(element)
	...
	if exports.factions:isPlayerInFaction( player, 8 ) then
		if esbarpt == 0 then
			exports.chat:me( player, "presiona el botón y sube la barrera." )
			moveObject ( barreraPT, 1200, 1416.1, 420, 19.6, 0, 90, 0)
			esbarpt = 1
			setTimer( function () 
			moveObject ( barreraPT, 1200, 1416.1, 420, 19.6, 0, -90, 0)
			end, 5000, 1, player)
...
--[[barreraDepo1 = createObject ( 968, 1400, 443.8, 19.9, 0, 270, 146.47 )
soporteDepo1 = createObject ( 966, 1400, 443.8, 19.1, 0, 0, 146.47 )
...
markerRadio = createMarker ( 2409.42, -38.55, 26.45, "cylinder", 7.5, 255, 0, 0, 0 )
esbarrad = 0
...
barreraTTL = createObject ( 968, 1242, 175, 20.09999961853, 0, 270, 336 )
markerTTL = createMarker ( 1238.7099609375, 176.505859375, 19.46875, "cylinder", 7.5, 255, 0, 0, 0 )
esbarttl = 0
...
function openBarreraTTL(element)
	...
	if exports.factions:isPlayerInFaction( player, 7 ) then
		if esbarttl == 0 then
			...
			exports.chat:me( player, "presiona el botón y sube la barrera." )
			moveObject ( barreraTTL, 1200, 1242, 175, 20.09999961853, 0, 90, 0)
		...
		elseif esbarttl == 2 then
			...
			local diff = 360-ry
			exports.chat:me( player, "presiona el botón y sube la barrera." )
			moveObject ( barreraTTL, 1200, 1242, 175, 20.09999961853, 0, diff, 0)
			esbarttl = 3
			setTimer( function ()
			esbarttl = 1 end, 1200, 1 )
		end
