	...
	if p[ player ] and isLoggedIn( player ) then
		...
		if getOption( player, "staffduty" ) then
			for key, value in ipairs( groups ) do
				local groupACL = aclGetGroup(value.aclGroup) or false
				if groupACL ~= false then
				     if isObjectInACLGroup( "user." .. p[ player ].username, groupACL) and value.nametagColor then
...
function getGroups( player )
	...
	if p[ player ] then
		for key, value in ipairs( groups ) do
			local groupACL = aclGetGroup(value.aclGroup) or false
			if groupACL ~= false then
				if isObjectInACLGroup( "user." .. p[ player ].username, groupACL) then
						...
						if groupinfo then
							...
							for key, group in ipairs( groups ) do
								...
								if not hasGroup then
									...
									local nameAclGroup = aclGetGroup(group.aclGroup)
									if nameAclGroup ~= false then
									     if aclGroupRemoveObject(nameAclGroup , "user." .. accountName ) then
