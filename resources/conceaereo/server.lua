local markerConceaereo = createMarker(1357.8001953125, 483.6865234375, 19.287950515747, "cylinder", 2, 0, 159, 10, 125)
markerConceaereo.dimension = 0

function guiConceaereo( player )
	if player and isElement(player) and getElementType(player) == "player" then
		...
		--if not getElementData(player, "abrirconce") == true then
			if hasObjectPermissionTo(player, 'command.vip', false) then
				triggerClientEvent(player, "abrirconceaereo", player, true)
			else
				triggerClientEvent(player, "abrirconceaereo", player, false)
			end
...
addEventHandler("onMarkerHit", markerConceaereo, guiConceaereo)
...
function comprarVehiculoaereo (comprador, model, precio, model2)
	if client and client == comprador then
		...
		outputChatBox("Usa /compraavion "..tostring(codigoCompra).." para confirmarlo (Tienes 15 segundos para hacerlo)", comprador, 255, 255, 0)
		setElementData(comprador, "codigoCompra", codigoCompra)
		setTimer(function (comprador, model2, precio, codigoCompra)
			if getElementData(comprador, "compraavion") == tostring(codigoCompra) then 
				if exports.players:takeMoney(comprador, precio) then
					regularObjetivosTrasCompra(comprador, precio)
					triggerEvent("generarVehiculoAereo", comprador, comprador, tonumber(model2))
					...
					removeElementData(comprador, "compraavion")
				...
				outputChatBox("La compra no ha sido autorizada. Usa /compraavion la próxima vez.", comprador, 255, 0, 0)
			end
		end, 15000, 1, comprador, model2, precio, codigoCompra, tip)
	end
end
addEvent( "comprarVehiculoaereo", true )
addEventHandler( "comprarVehiculoaereo", root, comprarVehiculoaereo )
 
function comprarVehiculoPrestamoaereo (comprador, model, precio, model2)
...
addEvent( "comprarVehiculoPrestamoaereo", true )
addEventHandler( "comprarVehiculoPrestamoaereo", root, comprarVehiculoPrestamoaereo )

function autorizarCompraavion(player, cmd, codigo)
	...
	if tostring(codigo) == tostring(codigoCompra) then
		...
		setElementData(player, "compraavion", tostring(codigoCompra))
	else
		outputChatBox("Los dos números del /compraavion no coinciden. Revisalo y prueba de nuevo.", player, 255, 0, 0)
	end
end
addCommandHandler("compraavion", autorizarCompraavion)
