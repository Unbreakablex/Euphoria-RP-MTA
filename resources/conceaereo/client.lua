...
function onRender()
	...
	elseif isCursorOver(sx*0.94, sy*0.955, sx*0.018, sy*0.022) then
		...
		if getKeyState('mouse1') and not click then
			...
			local vName = tabVeh[indexV].nReal or tabVeh[indexV].m
			triggerServerEvent("comprarVehiculoaereo", resourceRoot, getLocalPlayer(), vName, tonumber(tabVeh[indexV].p), tonumber(getVehicleModelFromName( tabVeh[indexV].m )), 1)
...
addEvent("abrirconceaereo", true)    
addEventHandler("abrirconceaereo", root, 
	function(isvp)
		
		isVIP = isvp
		indexV = 1
		tabVeh = exports.preciosaereo:getDatos() or {}
		isViewVeh = false

		vehTest = Vehicle(getVehicleModelFromName(tabVeh[indexV].m), 1942.7890625, -2409.5439453125, 14.170791625977 )
		vehTest.dimension = 0
		setElementFrozen(vehTest, 1000)
		setCameraMatrix( 1970.44140625, -2394.119140625, 17.337640762329, 1942.7890625, -2409.5439453125, 14.170791625977, 0, 70 )
        ----1366.83203125, 476.2607421875, 20.309841156006
        ---1352.107421875, 472.50390625, 23.287950515747
        
		 ---1981.44140625, -2394.119140625, 17.337640762329
        ---1942.7890625, -2409.5439453125, 14.170791625977
