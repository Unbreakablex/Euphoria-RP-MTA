...
function ame (source, causante, ...)
		if source and ... and causante then
			...
			if #message > 0 then
				localMessage( source, " *" .. message .. " ((" .. causante .. "))", 132, 255, 46 )
				return true
			else
			   return false
			end
...
function me( source, message, prefix )
	if isElement( source ) and getElementType( source ) == "player" and type( message ) == "string" then
		if prefix then
			...
			localMessage( source, message, 255, 58, 222 )	
		else
	...
	function( thePlayer, commandName, ... )
		if exports.players:isLoggedIn( thePlayer ) then
			...
			if #message > 0 then
				...
				local resto = string.sub(message, 2)
				local mensaje = tostring(pletra..resto)
				localMessage( thePlayer, " *" .. mensaje .. " ((" .. getPlayerName( thePlayer ) .. "))", 132, 255, 46 )
			else
...
local function pm( player, target, message, enc )
    ...
    if getElementData(target, "minpa") then
		triggerClientEvent(target, "onSendNotification", target, getPlayerName(player).." te ha enviado un PM en Exotic Juego de Rol. ¡Vuelve para leerlo!")
	end
	if enc then
...
addCommandHandler("ad",
	function( thePlayer, commandName, ... )
		if exports.players:isLoggedIn( thePlayer ) then
			...
			local message = table.concat( { ... }, " " )
			if #message > 0 and not getElementData(thePlayer, "nogui") == true then
				if getElementDimension(thePlayer) ~= 2 then outputChatBox("(( Debes de ir a una radio o enviar un SMS al 444 con el anuncio. ))", thePlayer, 255, 0, 0) return end
...
function(player)
	if exports.factions:isPlayerInFaction(player, 4) then
		if exports.players:takeMoney(player, 150) then
			...
			outputChatBox("No tienes 150 dólares para comprar la cámara.", player, 255, 0, 0)
		end	
	else
