		...
		if c then
			if p[ player ].route[ p[ player ].checkpoint ] then
				...
				if p[ player ].route[ p[ player ].checkpoint ].hint then
					triggerClientEvent( player, "gui:hint", player, "Tu trabajo: Pescador", p[ player ].route[ p[ player ].checkpoint ].hint )
				end
			end
			...
			if not n then
				...
				exports.objetivos:addObjetivo(3, exports.players:getCharacterID(player), player)
			else
...
local function createOurPedPescador( )
	if pedP then
		destroyElement( pedP )
	end
...
addEventHandler( "onPedWasted", resourceRoot, createOurPedPescador )
addEventHandler( "onResourceStart", resourceRoot, createOurPedPescador )

function desbugBot(player)
	setElementPosition(pedP, 2118.64, -92.9, 2.03, 308.37)
end
...
addEventHandler( "onElementClicked", resourceRoot,
	function( button, state, player )
		if button == "left" and state == "up" then
			...
			if getDistanceBetweenPoints3D( x, y, z, getElementPosition( source ) ) < 5 and getElementDimension( player ) == getElementDimension( source ) and source == pedP then
				triggerClientEvent("onAbrirJob", player, "pescador")
			end
		end
	end
)