...
local services =
{
	--[112] =
	{
		function( player, phoneNumber, input )
			if player then
				...
				return true, "Emergencias: Especifique si necesita si necesita policias ((/p SD)), si necesita médicos/bomberos ((/p FD))."
			end
		end,
		true,
		function( player, phoneNumber, input )
			if input[1] == "SD" or string.lower(input[1]) == "sd" then
				local policias = getJugadoresFaccion ( 1 )
				if #policias > 0 then
					sElegido[player] = 1
					return true, "Emergencias: San Andreas Sheriff Department, indique el motivo de su emergencia"
				else
					return false, "Emergencias: En este momento todos nuestros agentes están ocupados, ¡llame mas tarde!"
				end
			elseif input[1] == "FD" or string.lower(input[1]) == "fd" then
				local medicos = getJugadoresFaccion ( 2 )
				if #medicos > 0 then
					sElegido[player] = 2
					return true, "Emergencias: Red County Fire Department, indique el motivo de su emergencia"
				else
					return false, "Emergencias: En este momento no hay ningun médico/bombero disponible, ¡llame mas tarde!"
				end
			elseif input[1] == "MD" or string.lower(input[1]) == "md" then
				local bomberos = getJugadoresFaccion ( 2 )
				if #bomberos > 0 then
					sElegido[player] = 3
					return true, "Emergencias: Red County Fire Department, indique el motivo de su emergencia"
				else
					return false, "Emergencias: En este momento no hay ninguna unidad de emergencia disponible, ¡llame mas tarde!"
				end
			else
				return false, "Emergencias: No ha especificado el servicio, llame de nuevo al 911."
			end
		end,
		true,
		function( player, phoneNumber, input )
			...
			elseif sElegido[player] == 3 then
				...
				return false, "Emergencias: Un equipo de emergencia se dirige a su posicion, mantenga la calma."
			end
		end
	},
	[911] =
	...
	[555] =
	{
		function( player, phoneNumber, input )
			...
			if #mecanicos > 0 then
				return true, "PCMW operador dice: Ha llamado usted al servicio de talleres, explique brevemente el motivo de su llamada."
			else
				colgarLlamada(player)
				return false, "PCMW operador dice: En este momento no hay ningun mecanico disponible, ¡llame mas tarde!"
			end	
		end,
		true,
		function( player, phoneNumber, input )
			...
			if #mecanicos > 0 then
				...
				colgarLlamada(player)
				return false, "PCMW Operador dice: Un asistente de grua se dirige a su posicion, permanezca alli."
			else
				colgarLlamada(player)
				return false, "PCMW Operador dice: En estos momentos no hay ningun mecanico disponible, ¡llame mas tarde!"
			end
		end
	},
	[666] =
	{
		function( player, phoneNumber, input )
			...
			if #juzgados > 0 then
				return true, "Operador dice: Ha llamado usted al Gobierno de San Andreas , explique brevemente el motivo de su llamada."
			else
				colgarLlamada(player)
	...
	[333] =
	{
		function( player, phoneNumber, input )
			...
			if #juzgados > 0 then
				return true, "Operador dice: Ha llamado usted al Departamento de Justicia de Montgomery, explique brevemente el motivo de su llamada."
			else
				colgarLlamada(player)
				return false, "Operador dice: En este momento desde el Departamento de Justicia de Montgomery no podemos atenderle, ¡llame mas tarde!"
			end	
		end,
		true,
		function( player, phoneNumber, input )
			...
			if #juzgados > 0 then
				...
				colgarLlamada(player)
				return false, "Operador dice: De acuerdo, enseguida mandamos a quien corresponda del Departamento de Justicia de Montgomery"
			else
				colgarLlamada(player)
				return false, "Operador dice: En este momento desde el Departamento de Justicia de Montgomery no podemos atenderle, ¡llame mas tarde!"
			end
		...
		if ownNumber and otherNumber and message then
			if otherNumber ~= 112 and otherNumber ~= 911 and otherNumber ~= 444 then
				...
				if otherNumber == 444 then
					...
					if exports.players:takeMoney( player, 100 ) then
						...
						for key, value in ipairs( getElementsByType( "player" ) ) do
							...
							outputChatBox( "Se te ha cobrado 100 dólares por el envío del anuncio.", player, 0, 255, 0 )
							exports.logs:addLogMessage("anuncio", getPlayerName(player).." ha puesto un AD que dice: "..message.." .\n")
					else
						outputChatBox( "No tienes dinero suficiente para enviar un anuncio. Cuesta 100 dólares.", player, 255, 0, 0 )
					end
				else
...
function numerosDeServicio(player)
	outputChatBox("Servicios de Emergencias/Policias:911", player, 0, 255, 0)
	outputChatBox("Palomino Creek Motor Workshop: 555", player, 0, 255, 0)
	outputChatBox("Palomino Creek FM: 444", player, 0, 255, 0)
	outputChatBox("Gobierno de San Andreas: 666", player, 0, 255, 0)
	outputChatBox("Montgomery Court and Justice Dept: 333", player, 0, 255, 0)
end
